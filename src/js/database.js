...
(typeof $ == 'function') && $(function() {
	...
	var filter = function(init) {
		...
		var info = {
			normal_total_0: total_cnt,
			normal_total_4: addComma(formatFloat(total[0][4], 2)),
			normal_total_5: addComma(Math.floor(total[0][5], 0)),
			normal_total_6: addComma(Math.floor(total[0][6] / 1000000, 1)),
			normal_total_7: addComma(ratioRound(total[0][7], 1)),
		};
		for(var id in info) {
...
//		condition.searchflg = form.searchflg.options[form.searchflg.selectedIndex].value || false;
		filter(false);
		return false;
	};
	form.onreset = function() {
		condition.purpose = false;
		condition.area = false;
		condition.searchflg = false;
		filter(false);
		return true;
	}
